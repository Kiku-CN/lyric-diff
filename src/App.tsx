    ...
    <main className="shell">
      ...
      <section className="review-section">
        ...
        <div className="diff-table">
          ...
          {!isLoadingLyric && rows.map((row, index) => <div className={`diff-row ${row.kind}`} key={row.id}><div className="line-number">{String(index + 1).padStart(2, '0')}</div><div className="diff-cell local"><span>{row.localText || '—'}</span></div><div className="diff-cell reference">{row.referenceText ? <div className="reference-lines">{splitReferenceLines(row.referenceText).map((lineText, lineIndex) => { const lineKey = `${row.id}-${lineIndex}`; return <div className="reference-line" key={lineKey}><span>{lineText}</span><button className="copy-reference" onClick={() => copyReference(row, lineText, lineIndex)} aria-label={`复制第 ${index + 1} 行网易云歌词的第 ${lineIndex + 1} 句`}>{copiedLineKey === lineKey ? '已复制' : '复制'}</button></div>; })}</div> : <span>—</span>}</div><div className="row-controls"><button onClick={() => chooseRow(row, 'local')} className={row.chosenText === row.localText ? 'active' : ''} disabled={row.kind === 'add'}>现场</button><button onClick={() => chooseRow(row, 'reference')} className={row.chosenText === row.referenceText && Boolean(row.referenceText) ? 'active reference-choice' : ''} disabled={!row.referenceText || row.localIds.length > 1} title={row.localIds.length > 1 ? '多个 SRT 条目合并显示，参考文本仅用于对比' : undefined}>参考</button><input aria-label={`编辑第 ${index + 1} 行`} value={row.chosenText} disabled={row.localIds.length > 1} onChange={(event) => updateRow(row.id, { chosenText: event.target.value })} />{row.kind === 'add' && <label className="export-toggle"><input type="checkbox" aria-label={`导出第 ${index + 1} 行网易云歌词`} checked={Boolean(row.includeInExport)} onChange={(event) => updateRow(row.id, { includeInExport: event.target.checked })} />导出</label>}</div></div>)}
        </div>
      </section>

      <footer className="footer-note"><span>原始字幕不会被覆盖</span><span>·</span><span>网易云歌词仅作为参考</span><span>·</span><span>参考新增行按勾选结果导出</span></footer>
    </main>
  );
}
